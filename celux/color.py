...
def yuv_to_rgb(
    yuv_tensor: torch.Tensor, width: int, height: int, bit_depth: int = 8
) -> torch.Tensor:
    """
    ...
    if yuv_tensor.numel() < expected_size:
        raise ValueError(
            f"Tensor too small. Expected {expected_size}, got {yuv_tensor.numel()}"
        )

    # Slice planes
    y_plane = yuv_tensor[:y_size].view(height, width)
    u_plane = yuv_tensor[y_size : y_size + uv_size].view(uv_height, uv_width)
    v_plane = yuv_tensor[y_size + uv_size : y_size + 2 * uv_size].view(
        uv_height, uv_width
    )
    ...
    g = (
        y_shifted - 0.0722 * u_shifted - 0.6398 * v_shifted
    )  # Wait, checking coefficients

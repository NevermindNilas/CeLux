...
BatchDecoder::BatchDecoder(const Config& config)
    : config_(config)
{
    CELUX_DEBUG("BatchDecoder created: {}x{}x{}, dtype={}, device={}",
...
void BatchDecoder::seekToFrame(
    ...
    double fps)
{
    CELUX_TRACE("Seeking to frame {}", target_frame);
    ...
    if (ret < 0) {
        CELUX_WARN("Seek failed for frame {}, error code: {}", target_frame, ret);
    ...
    while (av_read_frame(fmt_ctx, pkt) >= 0) {
        if (pkt->stream_index == stream_idx) {
            ...
            while (ret >= 0) {
                ...
                if (ret < 0) {
                    CELUX_ERROR("Error decoding frame: {}", ret);
                    av_packet_unref(pkt);
                    av_packet_free(&pkt);
                    return false;
                }
                ...
                CELUX_TRACE("Decoded frame {}, target is {}", current_frame, target_frame);
...
void BatchDecoder::copyFrameToOutput(
    ...
    SwsContext* sws_ctx)
{
    CELUX_TRACE("Copying frame to {} positions", positions.size());
    ...
    if (sws_ctx) {
        ...
        // Create a temporary sws context if none provided
...
{
    CELUX_INFO("Decoding batch of {} frames", indices.size());
    ...
    CELUX_DEBUG("Decoding {} unique frames from {} total requests",
    ...
    try {
        for (int64_t target_frame : sorted_frames) {
            CELUX_TRACE("Processing target frame {}, current={}", target_frame, current_frame);
    ...
    CELUX_INFO("Batch decode completed successfully");
    return output;
}

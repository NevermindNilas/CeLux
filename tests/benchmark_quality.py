...
def generate_source():
    ...
    cmd = [
        "ffmpeg", "-y",
        "-f", "lavfi", "-i", f"testsrc=duration=2:size={WIDTH}x{HEIGHT}:rate={FPS}",
...
def pipeline_celux_ff():
    ...
    cmd = [
        ...
        "-c:v", "libx264",
        "-pix_fmt", "yuv420p",
        "-vf", "scale=in_range=pc:out_range=tv:out_color_matrix=bt709", # Explicit range handling
        "-frames:v", str(FRAMES),
        output
    ]
...
def pipeline_cv_cv():
    ...
    cap = cv2.VideoCapture(SOURCE_FILE)
    fourcc = cv2.VideoWriter_fourcc(*'mp4v') # 'avc1' often fails without openh264, using mp4v as fallback or 'X264' if available
...
def pipeline_ff_raw_rgb_ff():
    ...
    p2 = subprocess.Popen(encode_cmd, stdin=p1.stdout, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def main():
    ...
    for label, frames in pipelines_to_test:
        ...
        psnr_src, ssim_src = calculate_metrics(source_frames, frames, f"{label} vs Source")
        ...
        if label != "FF -> FF (Direct)":
             psnr_ff, ssim_ff = calculate_metrics(ff_ff_frames, frames, f"{label} vs FF->FF")

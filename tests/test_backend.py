...
def test_default_backend_returns_tensor():
    ...
    print(f"✓ Default backend: torch.Tensor with shape {frame.shape} and dtype {frame.dtype}")
...
def test_pytorch_backend_returns_tensor():
    ...
    print(f"✓ PyTorch backend: torch.Tensor with shape {frame.shape} and dtype {frame.dtype}")
...
def test_numpy_backend_returns_ndarray():
    ...
    print(f"✓ NumPy backend: numpy.ndarray with shape {frame.shape} and dtype {frame.dtype}")
...
def test_numpy_backend_iteration():
    ...
    max_frames = 10
    for frame in vr:
        assert isinstance(frame, np.ndarray), f"Expected numpy.ndarray in iteration, got {type(frame)}"
        count += 1
        if count >= max_frames:
            break

...
def generate_test_video(output_path: str, pixel_format: str, bit_depth: int,
    ...
    cmd = [
        ...
        "-colorspace", colorspace_map.get(color_space, "bt709"),
        "-color_range", range_map.get(color_range, "tv"),
    ]
...
def test_decode_with_celux(video_path: str, use_cuda: bool = True, num_frames: int = 60):
    """
...
def test_color_accuracy(video_path: str, expected_color_space: str):
    ...
    try:
        ...
        results["valid_range"] = (results["min_value"] >= 0 and results["max_value"] <= 255)
...
def run_format_tests(formats_to_test, temp_dir: str, use_cuda: bool = True):
    ...
    for pix_fmt, bit_depth, color_space, color_range, description in formats_to_test:
        ...
        try:
            ...
            # Generate test video
            video_path = os.path.join(temp_dir, f"test_{pix_fmt}_{bit_depth}bit_{color_space}_{color_range}.mp4")
            
            print(f"  Generating test video: {os.path.basename(video_path)}...")
            try:
                gen_success = generate_test_video(
                    video_path, pix_fmt, bit_depth, color_space, color_range,
                    width=1920, height=1080, duration=3.0, fps=30
                )
            ...
            try:
                decode_results = test_decode_with_celux(video_path, use_cuda=use_cuda, num_frames=90)
            except KeyboardInterrupt:
                print(f"  ⚠ Test interrupted by user")
                raise  # Re-raise keyboard interrupt to allow user to stop
            except Exception as decode_err:
                print(f"  ❌ Decoder exception: {type(decode_err).__name__}: {decode_err}")
            ...
            if decode_results and decode_results.get("success"):
                ...
                print(f"  ❌ Decoding failed: {decode_results.get('error', 'unknown error')}")
            else:
            ...
            if decode_results and decode_results.get("success"):
                ...
                if color_results and "error" not in color_results:
                    print(f"    Value range: [{color_results['min_value']}, {color_results['max_value']}]")
                    print(f"    Mean: {color_results['mean_value']:.1f}")
                    print(f"    Valid range: {'✓' if color_results['valid_range'] else '❌'}")
                    print(f"    Has contrast: {'✓' if color_results['has_contrast'] else '❌'}")
                elif color_results:
...
def compare_cpu_vs_cuda(temp_dir: str):
    ...
    try:
        ...
        if not generate_test_video(
            video_path, "yuv420p", 8, "bt709", "tv",
            width=1920, height=1080, duration=5.0, fps=30
        ):
        ...
        print("\nCPU Decoding:")
        try:
            cpu_results = test_decode_with_celux(video_path, use_cuda=False, num_frames=150)
            if cpu_results and cpu_results["success"]:
                ...
                print(f"  Failed: {cpu_results.get('error') if cpu_results else 'unavailable'}")
        except Exception as e:
        ...
        print("\nCUDA/NVDEC Decoding:")
        try:
            cuda_results = test_decode_with_celux(video_path, use_cuda=True, num_frames=150)
            if cuda_results and cuda_results["success"]:
                ...
                print(f"  Failed: {cuda_results.get('error') if cuda_results else 'unavailable'}")
        except Exception as e:
            print(f"  ❌ CUDA decoding exception: {type(e).__name__}: {e}")
            cuda_results = None
        
        # Speedup calculation
        if (cpu_results and cpu_results.get("success") and 
            cuda_results and cuda_results.get("success")):
...
""")
    ...
    try:
        ...
        generate_test_video(yuv420p_path, "yuv420p", 8, "bt709", "tv",
                           width=1920, height=1080, duration=2.0)
        ...
        try:
            yuv420p_results = test_decode_with_celux(yuv420p_path, use_cuda=True, num_frames=60)
...
def print_summary(results):
    ...
    for r in results:
        ...
        print(f"{r['format']:<20} {r.get('bit_depth', '?'):<6} {r.get('color_space', '?'):<10} {r.get('color_range', '?'):<8} {fps:<10} {status}")
...
def main():
    ...
    parser.add_argument("--quick", action="store_true", help="Run quick test with fewer formats")
    parser.add_argument("--keep-videos", action="store_true", help="Keep generated test videos")
    parser.add_argument("--output-dir", type=str, default=None, help="Output directory for test videos")
    parser.add_argument("--cpu-only", action="store_true", help="Test CPU decoding only")
